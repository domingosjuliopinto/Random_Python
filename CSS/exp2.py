...
def userinput():
    keyinp = input("Enter the key of 56 bits seperated by space: " "").strip().split()
    datainp = input("\nEnter the data of 64 bits to encrypt / decrypt seperated by space: " "").strip().split()
    #change to 56 later
    lenofkey = 56
    #change to 64 later
    lenofdata = 64
    if len(datainp) == lenofdata and len(keyinp) == lenofkey:
        print("The Data is accepted and loaded succesfully")
        print("The Key is accepted and loaded succesfully")
    else:
        while len(datainp) != lenofdata:
            print("Length of data entered ",len(datainp))
            datainp = input("Error in data entered. Enter the data of 64 bits to encrypt / decrypt seperated by space " "").strip().split()

        print("Data accepted, Data loaded succesfully")
        while len(keyinp) != lenofkey:
            print("Length of key entered ", len(keyinp))
            keyinp = input("Error in entered key. Enter the key (56 bits) to encrypt / decrypt seperated by space: " "").strip().split()
        print("Key accepted, Key loaded succesfully")
...
def main():
    key, data = userinput()
    operate = int(input("\nChoose 0 for encryption. \nChoose 1 for decryption.\n Enter your choice: "))
